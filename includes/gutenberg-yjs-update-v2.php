 ...
 * through Composer.
 *
 * @package Capital_P_Dangit_Gutenberg_Yjs_Update_V2
 */
...
const CPDANGIT_GUTENBERG_YJS_CONTENT_STRING = 4;
const CPDANGIT_GUTENBERG_YJS_CONTENT_TYPE   = 7;
const CPDANGIT_GUTENBERG_YJS_CONTENT_ANY    = 8;
const CPDANGIT_GUTENBERG_YJS_TYPE_ARRAY     = 0;
const CPDANGIT_GUTENBERG_YJS_TYPE_MAP       = 1;
const CPDANGIT_GUTENBERG_YJS_TYPE_TEXT      = 2;

/**
 * lib0-compatible string stream wrapper.
 *
 * yjs-php currently length-prefixes the string-length RLE payload. Yjs JS
 * expects those bytes directly after the concatenated string, so Capital_P_Dangit keeps
 * this tiny adapter until yjs-php exposes a compatible StringEncoder.
 */
class Capital_P_Dangit_Gutenberg_Yjs_String_Encoder {
	...
	public function write( string $value ): void {
		$this->buffer .= $value;
		$this->length_encoder->write( cpdangit_gutenberg_yjs_utf16_clock_len( $value ) );
	}

	public function to_string(): string {
		$encoder = new Encoding();
		$encoder->writeVarString( $this->buffer );
		cpdangit_gutenberg_yjs_encoding_write_bytes( $encoder, cpdangit_gutenberg_yjs_encoder_to_string( $this->length_encoder ) );
		return cpdangit_gutenberg_yjs_encoding_to_string( $encoder );
	}
}
 ...
 */
class Capital_P_Dangit_Gutenberg_Yjs_String_Decoder {
...
/**
 * Plain-array decoder for the updateV2 subset Capital_P_Dangit inspects.
 */
class Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Array_Decoder {
	...
	private Capital_P_Dangit_Gutenberg_Yjs_String_Decoder $string_decoder;
	...
	public function __construct( string $update ) {
		...
		$this->string_decoder      = new Capital_P_Dangit_Gutenberg_Yjs_String_Decoder( new Decoding( $decoder->readVarUint8Array() ) );
		...
		$this->rest_decoder        = new Decoding( cpdangit_gutenberg_yjs_decoder_remaining_data( $decoder ) );
	}
 ...
 */
class Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder {
	...
	private Capital_P_Dangit_Gutenberg_Yjs_Rle_Encoder $info_encoder;
	private Capital_P_Dangit_Gutenberg_Yjs_String_Encoder $string_encoder;
	private Capital_P_Dangit_Gutenberg_Yjs_Rle_Encoder $parent_info_encoder;
	...
	public function __construct() {
		...
		$this->info_encoder        = new Capital_P_Dangit_Gutenberg_Yjs_Rle_Encoder(
			static function ( Encoding $e, int $v ): void {
				$e->writeUint8( $v );
			}
		);
		$this->string_encoder      = new Capital_P_Dangit_Gutenberg_Yjs_String_Encoder();
		$this->parent_info_encoder = new Capital_P_Dangit_Gutenberg_Yjs_Rle_Encoder(
			static function ( Encoding $e, int $v ): void {
				$e->writeVarUint( $v );
			}
		);
	...
	public function to_string(): string {
		...
		$encoder->writeVarUint8Array( cpdangit_gutenberg_yjs_encoder_to_string( $this->key_clock_encoder ) );
		$encoder->writeVarUint8Array( cpdangit_gutenberg_yjs_encoder_to_string( $this->client_encoder ) );
		$encoder->writeVarUint8Array( cpdangit_gutenberg_yjs_encoder_to_string( $this->left_clock_encoder ) );
		$encoder->writeVarUint8Array( cpdangit_gutenberg_yjs_encoder_to_string( $this->right_clock_encoder ) );
		...
		$encoder->writeVarUint8Array( cpdangit_gutenberg_yjs_encoder_to_string( $this->type_ref_encoder ) );
		$encoder->writeVarUint8Array( cpdangit_gutenberg_yjs_encoder_to_string( $this->len_encoder ) );
		cpdangit_gutenberg_yjs_encoding_write_bytes( $encoder, cpdangit_gutenberg_yjs_encoding_to_string( $this->rest_encoder ) );
		return cpdangit_gutenberg_yjs_encoding_to_string( $encoder );
	}
}
 ...
 */
class Capital_P_Dangit_Gutenberg_Yjs_Rle_Encoder {
	...
	public function to_string(): string {
		$this->flush_count();
		return cpdangit_gutenberg_yjs_encoding_to_string( $this->encoder );
	}
 ...
 */
function cpdangit_gutenberg_yjs_encoder_to_string( object $encoder ): string {
 ...
 */
function cpdangit_gutenberg_yjs_encoding_to_string( Encoding $encoder ): string {
	return cpdangit_gutenberg_yjs_encoder_to_string( $encoder );
}

/**
 * Writes raw bytes with either the newer writeBytes() or older writeUint8Array().
 */
function cpdangit_gutenberg_yjs_encoding_write_bytes( Encoding $encoder, string $bytes ): void {
 ...
 */
function cpdangit_gutenberg_yjs_decoder_remaining_data( Decoding $decoder ): string {
 ...
 */
function cpdangit_gutenberg_yjs_decode_update_v2( string $update ): array {
	if ( ! class_exists( Decoding::class ) ) {
		throw new RuntimeException( 'maxschmeling/y-php is not loaded. Run composer install for Capital_P_Dangit.' );
	}

	$decoder       = new Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Array_Decoder( $update );
	...
	for ( $state_index = 0; $state_index < $state_count; $state_index++ ) {
		...
		for ( $i = 0; $i < $struct_count; $i++ ) {
			...
			if ( 0 === $content_ref ) {
				...
				$origin       = ( $info & 0x80 ) ? cpdangit_gutenberg_yjs_read_id( $decoder, true ) : null;
				$right_origin = ( $info & 0x40 ) ? cpdangit_gutenberg_yjs_read_id( $decoder, false ) : null;
				$parent       = null;
				$parent_sub   = null;
				...
				if ( null === $origin && null === $right_origin ) {
					...
					if ( $parent_info & 1 ) {
						...
						$parent = array_merge( array( 'type' => 'id' ), cpdangit_gutenberg_yjs_read_id( $decoder, true ) );
					}
				...
				$content = cpdangit_gutenberg_yjs_decode_item_content( $decoder, $content_ref );
 ...
 */
function cpdangit_gutenberg_yjs_read_id( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Array_Decoder $decoder, bool $left ): array {
	return array(
		'client' => $decoder->read_client(),
 ...
 * @return array<string, mixed>
 */
function cpdangit_gutenberg_yjs_decode_item_content( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Array_Decoder $decoder, int $content_ref ): array {
	switch ( $content_ref ) {
		case CPDANGIT_GUTENBERG_YJS_CONTENT_STRING:
			$text = $decoder->read_string();
			return array(
				'type'   => 'string',
				'value'  => $text,
				'length' => cpdangit_gutenberg_yjs_utf16_clock_len( $text ),
			);

		case CPDANGIT_GUTENBERG_YJS_CONTENT_TYPE:
			$type_ref = $decoder->read_type_ref();
			$types    = array(
				CPDANGIT_GUTENBERG_YJS_TYPE_ARRAY => 'Y.Array',
				CPDANGIT_GUTENBERG_YJS_TYPE_MAP   => 'Y.Map',
				CPDANGIT_GUTENBERG_YJS_TYPE_TEXT  => 'Y.Text',
			);
			return array(
				'type'     => 'type',
				'type_ref' => $type_ref,
		...
		case CPDANGIT_GUTENBERG_YJS_CONTENT_ANY:
			$length = $decoder->read_len();
			$values = array();
			for ( $i = 0; $i < $length; $i++ ) {
 ...
 */
function cpdangit_gutenberg_yjs_encode_single_paragraph_document_update_v2( int $client_id, string $paragraph_text, string $block_client_id ): string {
	$encoder = new Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder();
	...
	cpdangit_gutenberg_yjs_write_item( $encoder, 7, null, null, array( 'type' => 'root', 'key' => 'document' ), 'blocks', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_type_ref( 0 ); // Y.Array.
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 7, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => 0 ), null, function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_type_ref( 1 ); // Y.Map.
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 8, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => 1 ), 'name', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_len( 1 );
		$encoder->write_any( 'core/paragraph' );
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 7, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => 1 ), 'attributes', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_type_ref( 1 ); // Y.Map.
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 7, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => 3 ), 'content', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_type_ref( 2 ); // Y.Text.
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 4, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => 4 ), null, function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ) use ( $paragraph_text ): void {
		$encoder->write_string( $paragraph_text );
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 7, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => 1 ), 'innerBlocks', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_type_ref( 0 ); // Y.Array.
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 8, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => 1 ), 'clientId', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ) use ( $block_client_id ): void {
		$encoder->write_len( 1 );
 ...
 */
function cpdangit_gutenberg_yjs_encode_paragraph_insert_after_update_v2( int $client_id, string $paragraph_text, string $block_client_id, int $left_origin_client, int $left_origin_clock, int $start_clock = 0, ?array $right_origin = null, ?array $content_insert = null ): string {
	$encoder = new Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder();
	...
	cpdangit_gutenberg_yjs_write_item(
		$encoder,
		7,
		array(
		...
		null,
		null,
		function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
			$encoder->write_type_ref( 1 ); // Y.Map block.
		}
	);

	cpdangit_gutenberg_yjs_write_paragraph_block_fields( $encoder, $client_id, $paragraph_text, $block_client_id, $start_clock );
	...
	if ( $has_content_insert ) {
		...
		cpdangit_gutenberg_yjs_write_item(
			...
			null,
			function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ) use ( $content_insert ): void {
 ...
 */
function cpdangit_gutenberg_yjs_encode_text_replacement_update_v2( int $client_id, string $inserted_text, ?array $origin, ?array $right_origin, ?array $parent, array $delete_ranges, int $start_clock = 0 ): string {
	$encoder = new Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder();
	...
	cpdangit_gutenberg_yjs_write_item(
		...
		$parent
			...
			: null,
		null,
		function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ) use ( $inserted_text ): void {
 ...
 */
function cpdangit_gutenberg_yjs_write_paragraph_block_fields( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder, int $client_id, string $paragraph_text, string $block_client_id, int $block_clock ): void {
	...
	cpdangit_gutenberg_yjs_write_item( $encoder, 8, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => $block_clock ), 'name', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_len( 1 );
		$encoder->write_any( 'core/paragraph' );
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 8, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => $block_clock ), 'isValid', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_len( 1 );
		$encoder->write_any( true );
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 7, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => $block_clock ), 'attributes', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_type_ref( 1 ); // Y.Map.
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 7, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => $attributes_clock ), 'content', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_type_ref( 2 ); // Y.Text.
	} );

	if ( '' !== $paragraph_text ) {
		cpdangit_gutenberg_yjs_write_item( $encoder, 4, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => $content_type_clock ), null, function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ) use ( $paragraph_text ): void {
			$encoder->write_string( $paragraph_text );
		} );
	}

	cpdangit_gutenberg_yjs_write_item( $encoder, 8, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => $attributes_clock ), 'dropCap', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_len( 1 );
		$encoder->write_any( false );
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 7, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => $block_clock ), 'innerBlocks', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ): void {
		$encoder->write_type_ref( 0 ); // Y.Array.
	} );

	cpdangit_gutenberg_yjs_write_item( $encoder, 8, null, null, array( 'type' => 'id', 'client' => $client_id, 'clock' => $block_clock ), 'clientId', function ( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder ) use ( $block_client_id ): void {
		$encoder->write_len( 1 );
		$encoder->write_any( $block_client_id );
	} );
}
 ...
 */
function cpdangit_gutenberg_yjs_utf16_clock_len( string $value ): int {
	if ( '' === $value ) {
		return 0;
	}
 ...
 */
function cpdangit_gutenberg_yjs_paragraph_block_clock_len( string $paragraph_text ): int {
	return 8 + cpdangit_gutenberg_yjs_utf16_clock_len( $paragraph_text );
}

/**
 * Gets the number of Yjs client-clock ticks consumed by one inserted paragraph
 * block generated by cpdangit_gutenberg_yjs_encode_paragraph_insert_after_update_v2().
 */
function cpdangit_gutenberg_yjs_paragraph_insert_clock_len( string $paragraph_text, ?array $content_insert = null ): int {
	$length = cpdangit_gutenberg_yjs_paragraph_block_clock_len( $paragraph_text );
	if ( is_array( $content_insert ) && isset( $content_insert['text'] ) ) {
		$length += cpdangit_gutenberg_yjs_utf16_clock_len( (string) $content_insert['text'] );
	}
 ...
 *
 * @param Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder       Encoder.
 ...
 */
function cpdangit_gutenberg_yjs_write_item( Capital_P_Dangit_Gutenberg_Yjs_Update_V2_Encoder $encoder, int $content_ref, ?array $origin, ?array $right_origin, ?array $parent, ?string $parent_sub, callable $write_content ): void {
