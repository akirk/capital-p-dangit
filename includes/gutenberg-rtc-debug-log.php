 ...
 *
 * @package Capital_P_Dangit_Gutenberg_RTC_Debug_Log
 */
 ...
 */
function cpdangit_gutenberg_rtc_get_request_rooms( WP_REST_Request $request ): ?array {
 ...
 */
function cpdangit_gutenberg_rtc_summarize_rooms( array $rooms ): array {
	$summary = array();
 ...
 */
function cpdangit_gutenberg_rtc_decode_rooms_for_logging( array $rooms, callable $log ): array {
	$results = array();
	...
	foreach ( $rooms as $room_index => $room_request ) {
		...
		foreach ( $updates as $update_index => $update ) {
			$decoded = cpdangit_gutenberg_rtc_decode_update_entry( $update, $room, $client_id, $update_index );
 ...
 */
function cpdangit_gutenberg_rtc_decode_update_entry( $update, string $room, int $client_id, int $update_index ): array {
	$result = array(
		'room'         => $room,
		'client_id'    => $client_id,
	...
	$reader = new Capital_P_Dangit_Gutenberg_RTC_Debug_Update_Reader( $binary );

	$result['byte_length'] = strlen( $binary );
	$result['hex_prefix']  = bin2hex( substr( $binary, 0, 24 ) );

	if ( 'sync_step1' === $type || 'sync_step2' === $type ) {
		$result['sync_message'] = cpdangit_gutenberg_rtc_decode_sync_message_summary( $reader );
		$result['remaining']    = $reader->remaining();
		return $result;
	}

	if ( 'update' === $type || 'compaction' === $type ) {
		$result['y_update_v2'] = cpdangit_gutenberg_rtc_decode_update_v2_summary( $reader );
		try {
			$result['y_update_v2_structs'] = cpdangit_gutenberg_rtc_summarize_decoded_yjs_update( cpdangit_gutenberg_yjs_decode_update_v2( $binary ) );
 ...
 *
 * @param Capital_P_Dangit_Gutenberg_RTC_Debug_Update_Reader $reader Reader.
 * @return array<string, mixed>
 */
function cpdangit_gutenberg_rtc_decode_sync_message_summary( Capital_P_Dangit_Gutenberg_RTC_Debug_Update_Reader $reader ): array {
	$summary = array();
 ...
 * @return array<string, mixed>
 */
function cpdangit_gutenberg_rtc_decode_update_v2_summary( Capital_P_Dangit_Gutenberg_RTC_Debug_Update_Reader $reader ): array {
	$summary = array();
 ...
 * @return array<string, mixed>
 */
function cpdangit_gutenberg_rtc_summarize_decoded_yjs_update( array $decoded ): array {
	$items = array();
	...
	foreach ( $decoded['structs'] ?? array() as $struct ) {
		...
		$item    = array(
			...
			'origin'       => cpdangit_gutenberg_rtc_yjs_id_key( $struct['origin'] ?? null ),
			'right_origin' => cpdangit_gutenberg_rtc_yjs_id_key( $struct['right_origin'] ?? null ),
 ...
 */
class Capital_P_Dangit_Gutenberg_RTC_Debug_Update_Reader {
	private string $data;
