...
Incoming RTC updates are decoded with the copied Yjs/Gutenberg helpers from Shouter. The plugin tracks paragraph block state in `_cpdangit_room_state` and detects paragraph completion when Gutenberg creates the following empty top-level paragraph block. On completion, it runs WordPress core's `capital_P_dangit()` function against the paragraph text and maps any final-word change back into a Yjs replacement. Because core does not change bare lowercase `wordpress`, the plugin also has a narrow final-word fallback for that requested typing case.
...
- Top-level Gutenberg paragraph completion.
...
Deployable builds should include `vendor/`, matching the Shouter dist-branch pattern.
